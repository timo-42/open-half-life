...
fn a_worker_that_never_attests_readiness_times_out() {
    // The image is built (or fetched from its `OnceLock`) *before* the clock
    // starts. `image()` triggers a nested `cargo build` the first time it is
    // asked for a variant, and this test is the only user of `NeverReady`, so
    // it pays that cost in full — inside `started.elapsed()` if the call is
    // left inline, which measures the build rather than the deadline.
    let never_ready = image(TestWorkerVariant::NeverReady);
    let started = Instant::now();
    let outcome =
        launch_isolated_worker_from_image(&never_ready, deadline(Duration::from_millis(400))).err();
